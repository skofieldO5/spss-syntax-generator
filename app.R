  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        #Link to Tutorial
        a("Tutorial", href="https://www.berndschmidl.com/", target = "_blank")
      ),
      ...
      mainPanel(
        ...
        h4(id = "welcome", "Here you can generate your SPSS Syntax in a couple of seconds.
                      If you don't know what this is about, please click on Tutorial
                                 on the left."),
  ...
  server <- function(input, output) {
    ...
    observeEvent(input$syntax,  {
      ...
      if (is.null(input$uploaded_file)) {
        output$no_upload <- renderText("First, please upload an Excel file")
      } else {
      ...
      output$copy_button <- renderUI({
        ...
        table_to_copy <- substr(table_to_copy[-1],floor(length(table_to_copy)/10)+3,nchar(table_to_copy[1])) #get rid of the row numbers that come with print and the column names
